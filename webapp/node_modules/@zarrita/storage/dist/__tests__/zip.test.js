...
const store_v3_uncompressed_path = path.join(fixtures_dir, "v3", "data.uncompressed.zarr.zip");
...
describe("ZipFileStore.getRange", () => {
    ...
    it("reads partial data from a shard file", async () => {
        ...
        let indexSuffix = await store.getRange("/1d.contiguous.compressed.sharded.i2/c/0", { suffixLength: 20 });
        expect(indexSuffix).toEqual(fullBytes?.slice(-20));
        // Read first 10 bytes
        let prefix = await store.getRange("/1d.contiguous.compressed.sharded.i2/c/0", { offset: 0, length: 10 });
