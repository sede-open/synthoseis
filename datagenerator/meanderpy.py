    ...
    def plot_xsection(self, xsec, colors, ve):
        ...
        for i in range(0, ts - 1, 3):
            ...
            if self.model_type == "submarine":
                ax1.fill(
                    X1, Y1, facecolor=colors[2], linewidth=0.5, edgecolor=[0, 0, 0]
                )  # oxbow mud
                ax1.fill(
                    X1, Y2, facecolor=colors[0], linewidth=0.5, edgecolor=[0, 0, 0]
                )  # point bar sand
                ax1.fill(X1, Y3, facecolor=colors[1], linewidth=0.5)  # levee mud
            if self.model_type == "fluvial":
                ax1.fill(
                    X1, Y1, facecolor=colors[0], linewidth=0.5, edgecolor=[0, 0, 0]
                )  # levee mud
                ax1.fill(
                    X1, Y2, facecolor=colors[1], linewidth=0.5, edgecolor=[0, 0, 0]
                )  # oxbow mud
...
class ChannelBelt:
    ...
    def migrate(
        ...
        *D
        ...
        channel = self.channels[
            -1
        ]  # first channel is the same as last channel of input
        x = channel.x
        y = channel.y
        z = channel.z
        W = channel.W
        ...
        omega = (
            -1.0
        )  # constant in migration rate calculation (Howard and Knutson, 1984)
        gamma = 2.5  # from Ikeda et al., 1981 and Howard and Knutson, 1984
        for itn in trange(nit):  # main loop
            # update_progress(itn/nit)
            x, y = migrate_one_step(
                x, y, z, W, kl, dt, k, Cf, D, pad, pad1, omega, gamma
            )
            # x, y = migrate_one_step_w_bias(x,y,z,W,kl,dt,k,Cf,D,pad,pad1,omega,gamma)
            x, y, z, xc, yc, zc = cut_off_cutoffs(
                x, y, z, s, crdist, deltas
            )  # find and execute cutoffs
            x, y, z, dx, dy, dz, ds, s = resample_centerline(
                x, y, z, deltas
            )  # resample centerline
            slope = np.gradient(z) / ds
            ...
            if len(xc) > 0:  # save cutoff data
                self.cutoff_times.append(
                    last_cl_time + (itn + 1) * dt / (365 * 24 * 60 * 60.0)
                )
                cutoff = Cutoff(xc, yc, zc, W, D)  # create cutoff object
                self.cutoffs.append(cutoff)
            # saving centerlines:
            if np.mod(itn, saved_ts) == 0:
                self.cl_times.append(
                    last_cl_time + (itn + 1) * dt / (365 * 24 * 60 * 60.0)
                )
    ...
    def plot(self, plot_type, pb_age, ob_age, end_time, n_channels):
        ...
        if plot_type == "morph":
            ...
            pb_cmap = make_colormap(
                [green, green, pb_crit, green, pb_color, 1.0, pb_color]
            )  # colormap for point bars
            ob_cmap = make_colormap(
                [green, green, ob_crit, green, ob_color, 1.0, ob_color]
            )  # colormap for oxbows
            plt.fill(
                [xmin, xmax, xmax, xmin],
                [ymin, ymin, ymax, ymax],
                color=(106 / 255.0, 159 / 255.0, 67 / 255.0),
            )
        if plot_type == "age":
            ...
            if times[i] in sclt:
                ...
                if plot_type == "morph":
                    if times[i] > times[-1] - pb_age:
                        plt.fill(
                            xm,
                            ym,
                            facecolor=pb_cmap(i / float(len(times) - 1)),
                            edgecolor="k",
                            linewidth=0.2,
                        )
                    else:
                ...
                if plot_type == "age":
                    ...
                    plt.fill(
                        xm,
                        ym,
                        facecolor=age_cmap(i / float(n_channels - 1)),
                        edgecolor="k",
                        linewidth=0.1,
                        zorder=order,
                    )
        ...
        if plot_type == "age":
            ...
            plt.fill(
                xm, ym, color=(16 / 255.0, 73 / 255.0, 90 / 255.0), zorder=order
            )  # ,edgecolor='k')
        plt.axis("equal")
        plt.xlim(xmin, xmax)
        plt.ylim(ymin, ymax)
    ...
    def build_3d_model(
        ...
        if (
            (len(cot) > 0)
            & (len(np.where(cot >= starttime)[0]) > 0)
            & (len(np.where(cot <= endtime)[0]) > 0)
        ):
        ...
        if xmin == 0:  # plot centerlines and define model domain
            ...
            plt.plot(
                [pts[0, 0], pts[1, 0], pts[1, 0], pts[0, 0], pts[0, 0]],
                [pts[0, 1], pts[0, 1], pts[1, 1], pts[1, 1], pts[0, 1]],
                "r",
            )
        ...
        topo = np.zeros(
            (iheight, iwidth, 4 * n_steps)
        )  # array for storing topographic surfaces
        ...
        topoinit = (
            z1[0] - ((z1[0] - z1[-1]) / (xmax - xmin)) * xv * dx
        )  # initial (sloped) topography
        ...
        for i in trange(n_steps):
            ...
            # create distance map:
            cl_dist, x_pix, y_pix, z_pix, s_pix, z_map, x1, y1, z1 = dist_map(
                x, y, z, xmin, xmax, ymin, ymax, dx, delta_s
            )
            if i == 0:
            ...
            if model_type == "fluvial":
                ...
                if len(cutoff_ind) > 0:
                    cutoff_dists = 1e10 * np.ones(
                        np.shape(th)
                    )  # initialize cutoff_dists with a large number
                    for j in range(len(cutoff_ind)):
                        cutoff_dist, cfx_pix, cfy_pix = cl_dist_map(
                            cutoffs[int(cutoff_ind[j])].x[0],
                            cutoffs[int(cutoff_ind[j])].y[0],
                            cutoffs[int(cutoff_ind[j])].z[0],
                            xmin,
                            xmax,
                            ymin,
                            ymax,
                ...
                surf = (
                    surf + th_oxbows
                )  # update topographic surface with oxbow deposit thickness
                topo[:, :, 4 * i + 1] = surf  # top of oxbow mud
                facies[4 * i + 1] = 0
                surf = surf + th  # update topographic surface with sand thickness
                topo[:, :, 4 * i + 2] = surf  # top of sand
                facies[4 * i + 2] = 1
                surf = surf + mud_surface(
                    h_mud, levee_width / dx, cl_dist, w / dx, z_map, surf
                )  # mud/levee deposition
            ...
            if model_type == "submarine":
                surf = surf + mud_surface(
                    h_mud[i], levee_width / dx, cl_dist, w / dx, z_map, surf
                )  # mud/levee deposition
        ...
        topo = np.concatenate(
            (np.reshape(topoinit, (iheight, iwidth, 1)), topo), axis=2
        )  # add initial topography to array
        strat = topostrat(topo)  # create stratigraphic surfaces
        strat = np.delete(
            strat, np.arange(4 * n_steps + 1)[1::4], 2
        )  # get rid of unnecessary stratigraphic surfaces (duplicates)
        facies = np.delete(
            facies, np.arange(4 * n_steps)[::4]
        )  # get rid of unnecessary facies layers (NaNs)
        if model_type == "fluvial":
            facies_code = {0: "oxbow", 1: "point bar", 2: "levee"}
        if model_type == "submarine":
            facies_code = {0: "oxbow", 1: "channel sand", 2: "levee"}
        chb_3d = ChannelBelt3D(
            model_type, topo, strat, facies, facies_code, dx, channels3D
        )
...
def migrate_one_step(x, y, z, W, kl, dt, k, Cf, D, pad, pad1, omega, gamma):
    ...
    R0 = (
        kl * curv
    )  # simple linear relationship between curvature and nominal migration rate
...
def migrate_one_step_w_bias(x, y, z, W, kl, dt, k, Cf, D, pad, pad1, omega, gamma):
    ...
    x[pad1 : ns - pad + 1] = (
        x[pad1 : ns - pad + 1]
        + R1[pad1 : ns - pad + 1] * dy_ds * dt
        + T[pad1 : ns - pad + 1]
        * dy_ds
        * dt
        * (np.sin(np.deg2rad(angle)) * dx_ds + np.cos(np.deg2rad(angle)) * dy_ds)
    )
    y[pad1 : ns - pad + 1] = (
        y[pad1 : ns - pad + 1]
        - R1[pad1 : ns - pad + 1] * dx_ds * dt
        - T[pad1 : ns - pad + 1]
        * dx_ds
        * dt
        * (np.sin(np.deg2rad(angle)) * dx_ds + np.cos(np.deg2rad(angle)) * dy_ds)
...
def generate_initial_channel(W, D, Sl, deltas, pad, n_bends):
    ...
    if pad1 < 5:
        pad1 = 5
    x = np.linspace(
        0, noisy_len + (pad + pad1) * deltas, int(noisy_len / deltas + pad + pad1) + 1
    )  # x coordinate
    y = 10.0 * (
        2
        * np.random.random_sample(
            int(noisy_len / deltas) + 1,
        )
        - 1
    )
    y = np.hstack(
...
def compute_migration_rate(pad, ns, ds, alpha, omega, gamma, R0):
    ...
    for i in range(pad1, ns - pad):
        si2 = np.hstack(
            (np.array([0]), np.cumsum(ds[i - 1 :: -1]))
        )  # distance along centerline, backwards from current point
        G = np.exp(-alpha * si2)  # convolution vector
        R1[i] = omega * R0[i] + gamma * np.sum(R0[i::-1] * G) / np.sum(
            G
        )  # main equation
    return R1
...
def dist_map(x, y, z, xmin, xmax, ymin, ymax, dx, delta_s):
    ...
    img1 = morphology.binary_dilation(
        img, np.array([[1, 0, 1], [1, 1, 1]], dtype=np.uint8)
    ).astype(np.uint8)
    if len(np.where(img1 == 0)[0]) > 0:
        x_pix, y_pix = eliminate_bad_pixels(img, img1)
        x_pix, y_pix = order_cl_pixels(x_pix, y_pix)
    img1 = morphology.binary_dilation(
        img, np.array([[1, 0, 1], [0, 1, 0], [1, 0, 1]], dtype=np.uint8)
    ).astype(np.uint8)
    if len(np.where(img1 == 0)[0]) > 0:
        x_pix, y_pix = eliminate_bad_pixels(img, img1)
        x_pix, y_pix = order_cl_pixels(x_pix, y_pix)
...
def eliminate_bad_pixels(img, img1):
    ...
    img[y_ind : y_ind + 2, x_ind : x_ind + 2] = np.ones(
        1,
    ).astype(np.uint8)
