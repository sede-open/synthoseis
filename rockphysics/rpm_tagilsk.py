import numpy as np
from rockphysics.RockPropertyModels import RockProperties, store_1d_trend_dict_to_hdf
import sys
import os
# Add the parent directory to the system path
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
sys.path.insert(0, project_root)
from src.util.filehandler import load_chebyshev_approximation
from src.util.gaussian_processes import fast_chebyshev_evaluate

#3800 max depth
class RPMTagilsk():
    def __init__(self, cfg, *args, **kwargs):
        self.cfg = cfg
        self._shale_grid = None
        self._brine_grid = None
        self._oil_grid = None
        self._gas_grid = None

    def create_1d_trends(self, z = None, store_in_hdf = False):
        if z is None:
            z = np.arange(self.cfg.cube_shape[-1] * self.cfg.digi)
        
        d = dict(
            z=z,
            shale_vp=self.calc_shale_vp(z),
            shale_vs=self.calc_shale_vs(z),
            shale_rho=self.calc_shale_rho(z),
            brine_sand_vp=self.calc_brine_sand_vp(z),
            brine_sand_vs=self.calc_brine_sand_vs(z),
            brine_sand_rho=self.calc_brine_sand_rho(z),
            oil_sand_vp=self.calc_oil_sand_vp(z),
            oil_sand_vs=self.calc_oil_sand_vs(z),
            oil_sand_rho=self.calc_oil_sand_rho(z),
            gas_sand_vp=self.calc_gas_sand_vp(z),
            gas_sand_vs=self.calc_gas_sand_vs(z),
            gas_sand_rho=self.calc_gas_sand_rho(z),
        )
        if store_in_hdf:
            store_1d_trend_dict_to_hdf(self.cfg, d, z)

        return d
    
    @staticmethod
    def calc_shale_rho(z):
        return 7.7e-12 * z**3 + -8.8e-08 * z**2 + 0.0004 * z + 1.887

    @staticmethod
    def calc_shale_vp(z):
        return -0.00013 * z**2 + 1.13 * z + 1620

    @staticmethod
    def calc_shale_vs(z):
        return -0.0001 * z**2 + 0.96 * z + 579

    @staticmethod
    def calc_brine_sand_rho(z):
        return -7.8e-09 * z**2 + 0.00012 * z + 1.985

    @staticmethod
    def calc_brine_sand_vp(z):
        return -1.34e-05 * z**2 + 0.49 * z + 2197

    @staticmethod
    def calc_brine_sand_vs(z):
        return -1.0785e-05 * z**2 + 0.391 * z + 1157

    @staticmethod
    def calc_oil_sand_rho(z):
        return -9.23e-09 * z**2 + 0.00014 * z + 1.976

    @staticmethod
    def calc_oil_sand_vp(z):
        return -8.876e-06 * z**2 + 0.505 * z + 1838

    @staticmethod
    def calc_oil_sand_vs(z):
        return -1.126e-05 * z**2 + 0.391 * z + 656

    @staticmethod
    def calc_gas_sand_rho(z):
        return -1.818e-08 * z**2 + 0.000247 * z + 1.900

    @staticmethod
    def calc_gas_sand_vp(z):
        return -3.216e-06 * z**2 + 0.4796 * z + 1966

    @staticmethod
    def calc_gas_sand_vs(z):
        return -1.0687e-05 * z**2 + 0.3662 * z + 1135

    def calc_shale_properties(self, z_rho, z_vp, z_vs):

        cheb_params = load_chebyshev_approximation('../data/grids/chebyshev/shale_pl_vp_vs.json')
        mean_rho, _ = fast_chebyshev_evaluate(cheb_params, z_rho)
        mean_rho = mean_rho[:, 0]
        mask_rho = (z_rho >= 1250) & (z_rho <= 3800)
        rho = self.calc_shale_rho(z_rho)
        rho[mask_rho] = mean_rho[mask_rho]

        mean_vp, _ = fast_chebyshev_evaluate(cheb_params, z_vp)
        mean_vp = mean_vp[:, 1]
        mask_vp = (z_vp >= 1250) & (z_vp <= 3800)
        vp = self.calc_shale_vp(z_vp)
        vp[mask_vp] = mean_vp[mask_vp]

        mean_vs, _ = fast_chebyshev_evaluate(cheb_params, z_vs)
        mean_vs = mean_vs[:, 2]
        mask_vs = (z_vs >= 1250) & (z_vs <= 3800)
        vs = self.calc_shale_vs(z_vs)
        vs[mask_vs] = mean_vs[mask_vs]
        
        return RockProperties(rho, vp, vs)

    def calc_brine_sand_properties(self, z_rho, z_vp, z_vs):
        cheb_params = load_chebyshev_approximation('../data/grids/chebyshev/brine_pl_vp_vs.json')
        
        mean_rho, _ = fast_chebyshev_evaluate(cheb_params, z_rho)
        mean_rho = mean_rho[:, 0]
        mask_rho = (z_rho >= 1250) & (z_rho <= 3800)
        rho = self.calc_brine_sand_rho(z_rho)
        rho[mask_rho] = mean_rho[mask_rho]

        mean_vp, _ = fast_chebyshev_evaluate(cheb_params, z_vp)
        mean_vp = mean_vp[:, 1]
        mask_vp = (z_vp >= 1250) & (z_vp <= 3800)
        vp = self.calc_brine_sand_vp(z_vp)
        vp[mask_vp] = mean_vp[mask_vp]

        mean_vs, _ = fast_chebyshev_evaluate(cheb_params, z_vs)
        mean_vs = mean_vs[:, 2]
        mask_vs = (z_vs >= 1250) & (z_vs <= 3800)
        vs = self.calc_brine_sand_vs(z_vs)
        vs[mask_vs] = mean_vs[mask_vs]
        
        return RockProperties(rho, vp, vs)

    def calc_oil_sand_properties(self, z_rho, z_vp, z_vs):
        cheb_params = load_chebyshev_approximation('../data/grids/chebyshev/oil_pl_vp_vs.json')
        
        mean_rho, _ = fast_chebyshev_evaluate(cheb_params, z_rho)
        mean_rho = mean_rho[:, 0]
        mask_rho = (z_rho >= 1250) & (z_rho <= 3800)
        rho = self.calc_oil_sand_rho(z_rho)
        rho[mask_rho] = mean_rho[mask_rho]

        mean_vp, _ = fast_chebyshev_evaluate(cheb_params, z_vp)
        mean_vp = mean_vp[:, 1]
        mask_vp = (z_vp >= 1250) & (z_vp <= 3800)
        vp = self.calc_oil_sand_vp(z_vp)
        vp[mask_vp] = mean_vp[mask_vp]

        mean_vs, _ = fast_chebyshev_evaluate(cheb_params, z_vs)
        mean_vs = mean_vs[:, 2]
        mask_vs = (z_vs >= 1250) & (z_vs <= 3800)
        vs = self.calc_oil_sand_vs(z_vs)
        vs[mask_vs] = mean_vs[mask_vs]
        
        return RockProperties(rho, vp, vs)

    def calc_gas_sand_properties(self, z_rho, z_vp, z_vs):
        cheb_params = load_chebyshev_approximation('../data/grids/chebyshev/gas_pl_vp.json')
        
        # Gas grid has only 2 properties: rho and Vp
        mean_rho, _ = fast_chebyshev_evaluate(cheb_params, z_rho)
        mean_rho = mean_rho[:, 0]
        mask_rho = (z_rho >= 1250) & (z_rho <= 3800)
        rho = self.calc_gas_sand_rho(z_rho)
        rho[mask_rho] = mean_rho[mask_rho]

        mean_vp, _ = fast_chebyshev_evaluate(cheb_params, z_vp)
        mean_vp = mean_vp[:, 1]
        mask_vp = (z_vp >= 1250) & (z_vp <= 3800)
        vp = self.calc_gas_sand_vp(z_vp)
        vp[mask_vp] = mean_vp[mask_vp]

        # Vs uses polynomial model only (not in gas grid)
        vs = self.calc_gas_sand_vs(z_vs)
        
        return RockProperties(rho, vp, vs)